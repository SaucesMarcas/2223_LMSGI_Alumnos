...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:include href="../../../ud6/lib/libreria.xsl"/>
        ...
        <html>
            <head>               
                <xsl:call-template name="metaweb">
                    <xsl:with-param name="titulo"  select="concat('Carlos Distribución de software ',@tipo)"/>   
                    <xsl:with-param name="descripcion" select="'Listado de distribuciones de sotware libre '"/> 
                    <xsl:with-param name="autor" select="'Carlos Garcia Cachon'"/>
                </xsl:call-template>  
