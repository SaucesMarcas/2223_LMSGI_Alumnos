...
let $descuento := if ($precio/@moneda = 'euro') then xs:decimal($precio) * 0.15 else xs:decimal($precio) * 0.15 * 1.2
let $precio_descuento := if ($precio/@moneda = 'euro') then xs:decimal($precio) - $descuento else xs:decimal($precio) - $descuento * 0.8
order by $nombre
return
    <baile>
