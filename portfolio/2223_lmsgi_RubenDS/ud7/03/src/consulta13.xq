...
let $baile-laura := $bailes/baile[profesor[contains(., 'Laura')]]
let $precio := $baile-laura/precio
let $plazas := $baile-laura/plazas
let $ganancias := xs:decimal($precio) * xs:decimal($plazas)
return concat("Ganancias de Laura: ", $ganancias)